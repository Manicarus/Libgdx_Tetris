...
public class Tetrominoe {
	...
	Block[] blocks = new Block[BLOCK_NUM]; // A Tetrominoe consists of four Blocks // This is only creating 4 variables, not objects
	int curShape;
	int curColor;
	...
	Tetrominoe() { // Contructor
		for(int i = 0; i < BLOCK_NUM; i++) { // Creating objects for each variable
			blocks[i] = new Block();
		}
	 ...
	 * */
	void rotate() {
		int[][] tmpPos = new int[BLOCK_NUM][2];
		for(int i = 0; i < BLOCK_NUM; i++) {
			tmpPos[i][Block.X] = blocks[i].pos[Block.X];
			tmpPos[i][Block.Y] = blocks[i].pos[Block.Y];
		}
		
		for(int i = 0; i < BLOCK_NUM; i++) { // Move Tetrominoe to the origin and rotate
